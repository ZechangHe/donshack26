...
I also want to be honest about one thing. At the hackathon, I noticed that many of the other projects — some of which placed higher than ours — were more concept-driven. Some were games, some were mostly AI API calls. I'm not saying those are bad ideas, but I personally don't think a game truly helps the environment, and I don't think calling an AI to generate something is the same as solving a real problem. In fact, every AI API call requires GPU computation, which consumes a significant amount of electricity — and that electricity generates carbon emissions. So in an environment-themed hackathon, relying heavily on AI API calls is actually **working against the theme**.
